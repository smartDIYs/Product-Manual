...
| プリセットエリア | あらかじめ用意された日付形式から選択します。選択後に独自の形式へ変更することもできます。 |
...
| プリセットエリア | あらかじめ用意された時刻形式から選択します。選択後に独自の形式へ変更することもできます。 |
...
| 行間（mm）  | 行間の距離を設定します。 |
| 配置        | 複数行テキストの配置を設定します。デフォルトは「調整なし」です。 |
| すべて適用   | 変更した設定をすべてのテキスト要素に適用します。 |
...
| バージョン | QRコードのバージョン（セル数）を設定します。「Default」に設定すると、データ量に応じて適切なバージョンが自動的に選択されます。 |
| マスク | QRコードに適用するマスクパターンを設定します。0～7の8種類から選択できます。 |
| フォーマット | Data Matrixのフォーマットを設定します。 |
| 高さ | QRコードの高さをmm単位で設定します。 |
| 中央空白サイズ | QRコードの中央部分を削除する範囲を設定します。 |
...
* 行番号・列番号・Xオフセット・Yオフセット: 行番号と列番号で配列内のオブジェクトを指定し、X方向・Y方向の位置を個別に調整します。
...
<td>一方向: 各走査線を常に左から右の方向にマーキングします。</td>
</tr>
<tr>
