...
MarkSpeed（マーキング速度）、JumpSpeed（ジャンプ速度）、Power（パワー）、PW（パルス幅）、MopaPW（Mopaパルス幅）、Freq（周波数）、PointTime（ドット時間）などのパラメータを取得できます。<br>
返信例では、4000がマーキング速度、80がパワーを示します。
</div>
</div>
...
<div class="annotation">
ID,0：ペン番号0を指定します。<br>
MarkSpeed,4000：マーキング速度を4000に設定します。<br>
