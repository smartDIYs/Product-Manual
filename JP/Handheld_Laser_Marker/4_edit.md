...
| 文字間隔 | 文字同士の間隔（mm）を指定します。「等間隔」を有効にすると文字の配置間隔を指定できます。 |
...
| サイクル | 有効の場合、加工中にシリアル番号が終了番号に達した時に自動的に開始番号にリセットされ、加工を継続します。 |
| Static | <TODO:メーカー確認。> |
...
| 名称 | <TODO:メーカー確認> |
| 保証期間（日）（Warranty period（days）） | この期間中に日付／時刻を編集する場合、システムは自動的にカスタム形式を既定の設定として使用します。<TODO:挙動確認> |
...
<div class="annotation">
例：フォーマットを「***aaaAAA」に設定した場合、3桁の数字・3文字の小文字・3文字の大文字を組み合わせたランダムコード（495rcjECS / 031pedWMK など）が生成されます。<br>
</div>


<TODO:メーカー確認。バグあり、マニュアル記載の上記フォーマットでは文字が指定種別にならず、完全ランダムになる>
...
## 遅延機
複数のオブジェクトを加工する場合、各オブジェクトの加工完了から次のオブジェクトの加工開始までに待ち時間（タイマー）を挿入できます。

「遅延機」をタップすると編集画面が表示され、遅延時間を設定できます。
必要に応じてオブジェクトリストなどで加工順序を変更し、遅延を入れたいオブジェクトの直前に「遅延機」を配置してください。
